      ...
      const productsWithNames = productsData.map(product => {
        ...
        return {
          ...
          offerPrice: Number(
            ...
            product.offerprice ??
            attrs['Offer Price'] ??
            attrs['Price to BI incl of taxes'] ??
            attrs['Courier Final Price to BI'] ??
            0
          ),
          ...
          // SPECIFICATIONS
          weight: product["weight(g)"] ?? null,
          height: product["height(cm)"] ?? null,
          width: product["width(cm)"] ?? null,
          length: product["length(cm)"] ?? null,
...
useEffect(() => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    // Filter by view mode (featured or all)
...
if (searchTerm) {
  ...
  filtered = filtered.filter(product =>
    ...
    String(product.sellerid ?? '').toLowerCase().includes(term) ||
...
if (selectedCategory !== 'all') {
  filtered = filtered.filter(
    ...
    return filtered;
  }, [products, searchTerm, selectedCategory, selectedStatus, sortBy, sortOrder, viewMode]);
