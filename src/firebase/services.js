...
export const orderService = {
  ...
  async getAll() {
    try {
      ...
      // Fetch all orders across users using collectionGroup
      const ordersGroupRef = collectionGroup(db, 'orders');
      const ordersSnapshot = await getDocs(ordersGroupRef);

      for (const orderDoc of ordersSnapshot.docs) {
        const orderData = orderDoc.data();
        const userRef = orderDoc.ref.parent.parent; // users/{userId}
      ...
      // Include cancelled orders across users using collectionGroup
...
console.log(
  `Total orders found: ${orders.length}, Unique orders: ${uniqueOrders.length}`
);
...
return uniqueOrders;
  ...
  async getByStatus(status) {
    try {
      ...
      const [snapStatus, snapOrderStatus] = await Promise.all([
        getDocs(qStatus),
        getDocs(qOrderStatus)
      ]);

      const processedIds = new Set();
      const processDocs = async (snapshot) => {
        for (const orderDoc of snapshot.docs) {
          if (processedIds.has(orderDoc.id)) continue;
          processedIds.add(orderDoc.id);
          ...
          const fullOrder = {
            ...
            shippingAddress,
            address: orderData.address || shippingAddress,
            status: orderData.status || orderData.orderStatus || status
          };
          orders.push(fullOrder);
          this._cacheSet(fullOrder.id, fullOrder);
        }
      ...
      await processDocs(snapStatus);
      await processDocs(snapOrderStatus);
